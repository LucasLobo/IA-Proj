    ...
    def traces2Q(self, trace):
        ...
        while True:
            for step in trace:
                ...
                final = int(step[2])
                reward = step[3]
                self.Q[initial][action] += alpha * (reward + self.gamma * np.amax(self.Q[final]) - self.Q[initial][action])

            threshold = np.sqrt(sum(sum((self.Q-auxQ)**2)))
            if threshold < 0.0001:
                break

            auxQ = copy.deepcopy(self.Q)
